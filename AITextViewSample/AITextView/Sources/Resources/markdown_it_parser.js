// ==================== Markdown-it 解析器 ====================
// 负责：
// 1. 初始化 markdown-it 及其插件，确保表格/emoji/代码复制/数学/上下标等扩展语法可用
// 2. 将 Markdown 文本解析成 markdown-it tokens（由 markdown-it 完成）
// 3. 将 tokens 转换成自定义 AST（通过 tokensToAST + 各 parseXxx 方法），供 PureJSMarkdownRenderer 使用
// 简单理解：Markdown -> tokens -> AST，这个文件完成前两步的 glue 逻辑
class MarkdownItParser {
  ...
  tokensToAST(tokens) {
    // 将 markdown-it 生成的 tokens 顺序扫描并转换为自定义 AST 数组
    const ast = []
    let i = 0
...
/*
startIndex：当前标题的起始下标，指向 tokens 数组里对应 heading_open token 的位置。函数会从这个位置开始，
向后找到与之匹配的 heading_close、inline 等 token，以确定标题等级、内容，并最终返回 AST 节点和消费到的范围。
*/
